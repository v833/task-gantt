...
export default class StyleBox {
  ...
  private __fullMode: Boolean = false;
  public get fullMode(): Boolean {
    return this.__fullMode;
  }

  public set fullMode(v: Boolean) {
    this.__fullMode = v;
  }
  ...
  private _mileStones: Array<{ date: XDate[]; color?: string }> = [];
  public get mileStones(): Array<{ date: XDate[]; color?: string }> {
    return this._mileStones;
  }

  public set mileStones(
    mileStones: Array<{ date: LikeDate | LikeDate[]; color?: string }>
  ) {
    if (this._mileStones.length > 0) return;
    const v = cloneDeep(mileStones);
    const h = v.map(item => {
      if (!Array.isArray(item.date)) {
        item.date = [item.date];
      }
      const date = cloneDeep(
        item.date.map(d => {
          return new XDate(d);
        })
      );
      return {
        date,
        color: item.color ?? this.bodyStyle?.weekendColor ?? '#F53F3F'
      };
    });
