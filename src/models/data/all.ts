...
export default class AllData {
  ...
  private __createRow(
    ...
    // 更新两头的日期
    this.__updateDate(row, options.endOffset);
   ...
   * 数据全部展开/闭合
   */
  updateExpand(ids: string[]) {
    this.__expandIds = ids;
    const fn = (data: RowItem[]) => {
      data.forEach(row => {
        row.setExpand(ids.includes(row.id));
  ...
  update(data: any[], options: DataOptions = {}) {
    options.expandIds = this.expandIds || [];
    this.__level = 0;
